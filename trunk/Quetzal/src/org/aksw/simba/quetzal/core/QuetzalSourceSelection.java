...
import org.aksw.simba.quetzal.datastructues.HyperGraph.HyperEdge;
import org.aksw.simba.quetzal.datastructues.HyperGraph.Vertex;
...
/**
 * Perform triple pattern-wise source selection using Quetzal Sumaries, cache and SPARQL ASK. 
...
public class QuetzalSourceSelection {
	 ...
	 */
	public  Map<StatementPattern, List<StatementSource>> performSourceSelection(HashMap<Integer, List<StatementPattern>> bgpGroups ) throws RepositoryException, MalformedQueryException, QueryEvaluationException 
	{
		long tp = 0 ;
		for(int DNFkey:bgpGroups.keySet())  //DNFgrp => bgp
		{
			...
			List<StatementPattern>	 stmts = bgpGroups.get(DNFkey);
				...
				for (Vertex v:V) //vertex
				{
					...
					if((v.inEdges.size()>1 && v.outEdges.size()>0) || (v.inEdges.size()>0 && v.outEdges.size()>1)) 
					{
						...
						for(HyperEdge inEdge: v.inEdges) //has hyperedges or statement patterns
						{
							...
							{
								authIntersectionSet = getAuthorities(authUnionSet);
							}
							else
							{
		...
		int newSources = 0;
		 for(StatementPattern stmt: stmtToSources.keySet())
		  newSources = newSources + stmtToSources.get(stmt).size();
			System.out.println("Total Triple pattern-wise sources selected : "+ newSources);

		
         
		return stmtToSources;
	}
	private ArrayList<String> getAuthorities(ArrayList<String> uriSet) {
	 ...
	 */
	public static ArrayList<String> getUnion(ArrayList<String> authUnionSet,	ArrayList<String> lstAuthorities)
	{
	...
	//System.out.println( stmtToLstAuthorities);
		...
		Map<StatementPattern, Map<StatementSource, ArrayList<String>>> newStmtToLstAuthorities = new ConcurrentHashMap <StatementPattern, Map<StatementSource, ArrayList<String>>> ();
		...
		ArrayList<String> allNewURIs = new ArrayList<String> () ;
		for(StatementPattern stmt:stmtToLstAuthorities.keySet())
		{
			Map<StatementSource, ArrayList<String>> newStmtSourceToLstAuthorities = new HashMap<StatementSource, ArrayList<String>>();
		// System.out.println("\n\n");
		// System.out.println("Intersection set: "+authIntersectionSet);
			...
			{
				// System.out.println("\n");
				ArrayList<String> srcAuthSet = getAuthorities(stmtSourceToLstAuthorities.get(src));
				srcAuthSet.retainAll(authIntersectionSet);
				
				if(srcAuthSet.size()>0)  //consider the sources and statements for the second iteration of source prunning. this time including outside part of uri authority
				{
				//System.out.println("\n"+src.getEndpointID()+ ": "+srcAuthSet+": "+ stmt.getPredicateVar().getSignature() );
				ArrayList<String> remURIs = stmtSourceToLstAuthorities.get(src);
	...
	private Map<StatementPattern, Map<StatementSource, ArrayList<String>>> addAllCombinations(Map<StatementPattern, Map<StatementSource, ArrayList<String>>> newStmtToLstAuthorities,
			ArrayList<String> allNewURIs) 
	   {
		Map<StatementPattern, Map<StatementSource, ArrayList<String>>> hm= new HashMap<StatementPattern, Map<StatementSource, ArrayList<String>>>();
		Trie.constructTrie(allNewURIs, 30);
		for(StatementPattern stmt:newStmtToLstAuthorities.keySet())
		{
			Map<StatementSource, ArrayList<String>> newStmtSourceToLstAuthorities = new HashMap<StatementSource, ArrayList<String>>();
			Map<StatementSource, ArrayList<String>> stmtSourceToLstAuthorities = newStmtToLstAuthorities.get(stmt);
			for(StatementSource src:stmtSourceToLstAuthorities.keySet())
			{
				ArrayList<String> srcURISet = stmtSourceToLstAuthorities.get(src);
				ArrayList<String> newSrcURISet = new ArrayList<String> ();
				for(String uri:srcURISet)
				{
					newSrcURISet = getUnion(newSrcURISet,(ArrayList<String>) Trie.getAllCombinations(uri));
				}
	...
	private void doFinalPruning(
			Map<StatementPattern, Map<StatementSource, ArrayList<String>>> newStmtToLstAuthorities) {
       // System.out.println("\nAfter : "+ newStmtToLstAuthorities);

		ArrayList<String> intersectionSet = getIntersection(newStmtToLstAuthorities);
	...
	private ArrayList<String> getIntersection(
		...
		{
			Map<StatementSource, ArrayList<String>> stmtSourceToLstAuthorities = newStmtToLstAuthorities.get(stmt);
			ArrayList<String> stmtURIs = new ArrayList<String> ();  //get all URIs (over all relevant sources) of a stmt
			for(StatementSource src:stmtSourceToLstAuthorities.keySet())
			{
				ArrayList<String> srcURISet = stmtSourceToLstAuthorities.get(src);
	...
	protected static class SourceSelectionExecutorWithLatch implements ParallelExecutor<BindingSet> {
		 ...
		 * @param QuetzalSourceSelection Source Selection
