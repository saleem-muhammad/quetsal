...
import org.aksw.simba.quetzal.core.Prefix;
...
//		 //System.out.println(uRIAllCombinations);
	...
	// words.clear();
		...
		//  dfs(TrieNode.root,0);
		    ...
		    Trie.constructTrie(uris,30);
		   System.out.println( getAllCombinations("http://data.linkedmdb.org/resource/"));
...
//		    }
	...
	public static ArrayList<String> getAllBranchingURIs(Map<String, List<String>> authGroups,int branchLimit) {
	...
	public static List<String> getAllCombinations(String uRI) {
	...
	public static List<String> getRoot2StopingNodePaths (TrieNode root,int branchLimit) {
		  // assume root != NULL
		paths.clear();
	 getRoot2StopingNodePaths(root, new LinkedList<TrieNode>(),branchLimit);
	 return paths;
		}

		private static void getRoot2StopingNodePaths(TrieNode root, LinkedList<TrieNode> path,int branchLimit) {
		  ...
		  if (root.children.size()==0) {
		    ...
		    for (TrieNode node: root.children.values()) {
		    	getRoot2StopingNodePaths (node, new LinkedList<TrieNode>(path),branchLimit);
		    }
		  }
	...
	public static TrieNode constructTrie(List<String> uRIs,int branchLimit) {
	...
	static void dfs(TrieNode node, int depth){
		if(node.children.size()>1)
	    	   isPrefixedFound=true;
	    for (TrieNode tn: node.children.values())
	    {
	       System.out.println("node: "+ tn.letter+"  branches:" + tn.children.size()+" prent:"+ tn.parent.letter);
	       if(isPrefixedFound==false)
	       lcp=lcp+tn.letter;
	    	dfs(tn,depth+1);
	    }
	}
